	...
	public sealed partial class Plugin : BasePlugin
	{
		...
		public ArenaPlayer? SetupPlayer(CCSPlayerController playerController)
		{
			...
			try
			{
				...
				arenaPlayer.ArenaTag = $"{Localizer[gameRules?.WarmupPeriod == true ? "zhw.general.warmup" : "zhw.general.waiting"]} |";
				...
				{
					arenaPlayer.Controller.PrintToChat($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.chat.queue_added", WaitingArenaPlayers.Count]}");
					arenaPlayer.Controller.PrintToChat($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.chat.arena_afk", Config.CommandSettings.AFKCommands.FirstOrDefault() ?? "Missing"]}");

					if (HasDatabase)
					{
						arenaPlayer.Controller.PrintToChat($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.chat.arena_commands", Config.CommandSettings.GunsCommands.FirstOrDefault() ?? "Missing", Config.CommandSettings.RoundsCommands.FirstOrDefault() ?? "Missing"]}");
		...
		public bool CommandHelper(CCSPlayerController? player, CommandInfo info, CommandUsage usage, int argCount = 0, string? help = null, string? permission = null)
		{
			switch (usage)
			{
				case CommandUsage.CLIENT_ONLY:
					...
					{
						info.ReplyToCommand($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.general.commandclientonly"]}");
						return false;
					}
					break;
				case CommandUsage.SERVER_ONLY:
					if (player != null)
					{
						info.ReplyToCommand($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.general.commandserveronly"]}");
						return false;
					}
					break;
			...
			if (permission != null && permission.Length > 0)
			{
				...
				{
					info.ReplyToCommand($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.general.commandnoperm"]}");
					return false;
				}
			}
			...
			if (argCount > 0 && help != null)
			{
				...
				{
					info.ReplyToCommand($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.general.commandhelp", info.ArgByIndex(0).Replace("css_", string.Empty), help]}");
					return false;
				}
			}
		...
		public string GetOpponentNames(List<ArenaPlayer>? opponents)
		{
			if (opponents is null || opponents.Count == 0)
				return Localizer["zhw.general.no_opponent"];
		...
		public string? GetRequiredTag(CCSPlayerController player)
		{
			...
			if (WaitingArenaPlayers.Any(p => p.Controller == player))
				return Localizer["zhw.general.waiting"];
		...
		public string GetRequiredTag(int arenaID) =>
			arenaID switch
			{
				-2 => $"{Localizer["zhw.general.challenge"]} |",
				-1 => $"{Localizer["zhw.general.warmup"]} |",
				_ => $"{Localizer["zhw.general.arena"]} {arenaID} |",
			};
