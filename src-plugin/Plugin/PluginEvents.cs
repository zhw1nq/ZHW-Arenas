		...
		public void Initialize_Events()
		{
			...
			RegisterEventHandler((EventRoundPrestart @event, GameEventInfo info) =>
			{
				...
				foreach (ArenaPlayer player in validPlayers)
				{
					if (player.AFK)
					{
						player.Controller.PrintToChat($"{Localizer["zhw.general.prefix"]} {Localizer["zhw.chat.afk_reminder", Config.CommandSettings.AFKCommands.FirstOrDefault("Missing")]}");

						player.ArenaTag = $"{Localizer["zhw.general.afk"]} |";
					...
					if (challengeList.Count > 0)
					{
						...
						if (!player1IsAvailable || !player2IsAvailable)
						{
							if (challenge.Player1.IsValid)
								challenge.Player1.Controller.PrintToChat($"{Localizer["zhw.general.prefix"]} {Localizer["zhw.general.challenge.cancelled"]}");

							if (challenge.Player2.IsValid)
								challenge.Player2.Controller.PrintToChat($"{Localizer["zhw.general.prefix"]} {Localizer["zhw.general.challenge.cancelled"]}");
