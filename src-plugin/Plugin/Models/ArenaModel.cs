	...
	public void SetPlayerDetails(List<ArenaPlayer>? team, List<SpawnPoint> spawns, CsTeam switchTo, List<ArenaPlayer>? opponents)
	{
		...
		foreach (ArenaPlayer player in team)
		{
			if (player?.IsValid == true)
			{
				...
				if (ArenaID != -1)
				{
					player.Controller.PrintToChat($" {Localizer["zhw.general.prefix"]} {Localizer["zhw.chat.arena_roundstart", Plugin.GetRequiredArenaName(ArenaID), Plugin.GetOpponentNames(opponents) ?? "Unknown", Localizer[RoundType.Name ?? "Missing"]]}");
				}
		...
		else
		{
			...
			else
			{
				...
				if (team1Alive == team2Alive)
				{
					if (ArenaID == -2)
					{
						Server.PrintToChatAll($"{Localizer["zhw.general.prefix"]} {Localizer["zhw.general.challenge.tie", Team1.First().Controller.PlayerName, Team2.First().Controller.PlayerName]}");
				...
				else
				{
					if (ArenaID == -2)
					{
						...
						Server.PrintToChatAll($"{Localizer["zhw.general.prefix"]} {Localizer["zhw.general.challenge.winner", winner.Controller.PlayerName, loser.Controller.PlayerName]}");
